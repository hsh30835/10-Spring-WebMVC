...
@Controller //사용자의 요청을 받아주는 역할을 수행함
...
public class FirstController {
    ...
    *     WebRequest는 Spring의 일부이기 떄문에 Spring 기반의 프로젝트에서 더 자주 사용 된다.
    * */
    @PostMapping("regist")
    public String registMenu(Model model, WebRequest request){
        ...
        // 해당 view이름을 리턴받는데 messagePrinter + suffix = "html"인데 넘겨주면 messagePrinter + "html"로 변환됨
        // 그래서 return "first/messagePrinter.html"로 안하는것
        // 지금만든걸 html에 넣어줘서 매칭해야됨
    }
     ...
     * */
    @PostMapping("modify")
    public String modifyMenuPrice(Model model, @RequestParam(required = false) String modifyName,/*값을 생략하면 빈문자열이 넘어감*/
    ...
    * */
    ...
    public String modifyMenu(Model model, @RequestParam Map<String, String> parameters){
        //Http프로토콜은 값을 받을때 전부 String으로 받음
        String modifyMenu = parameters.get("modifyMenu2");
    ...
    * 클래스렙레에 @SessionAttributes 어노테이션을 이용하여 세션에 값을 담을 key를 설정해 두면
    ...
    * 해당 어노테이션은 http 본문 잘체를 읽는 부분을 모델로 변환시켜 주는 어노테이션이다.
    *
    ...
    * @CookieValue를 이요해서 쿠키 정보도 쉽게 불러올 수 있다.
    * */
    @PostMapping("body")
